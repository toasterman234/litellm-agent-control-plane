...
This directory is a liftable skeleton for a dedicated `central-repo-ops` repository.

It is intentionally lightweight:

- workflows are valid but conservative
- manifests are JSON so the helper scripts run without extra dependencies
- repo writes default to dry-run behavior
- the target-repo worker is included as a reference, not wired into this LAP repo
...
- Replace sample repos with your pilot set.
