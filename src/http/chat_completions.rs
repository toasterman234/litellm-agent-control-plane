...
pub async fn chat_completions(
    ...
    send_chat_completions_request(state, headers, body, model, route).await
}
...
async fn send_chat_completions_request(
    ...
    let prepared = route.handler.transform_chat_completions_request(
        body.clone(),
        &route.deployment,
        &headers,
    )?;
    ...
    build_chat_completions_response(state, route, upstream, stream, payload).await
}
...
async fn send_upstream(
    ...
    let upstream = match llm::send_request(
        &state.http,
        route.handler.chat_completions_url(&route.deployment),
        prepared,
    )
    .await
    {
...
async fn build_chat_completions_response(
    state: Arc<AppState>,
    route: Route,
    ...
    let response_headers = route
        .handler
        .transform_chat_completions_response_headers(upstream.headers(), stream);
    if route.handler.transforms_chat_completions_response_body() {
        return llm::build_logged_transformed_response(
            ...
            |body, status, content_type| {
                route.handler.transform_chat_completions_response_body(
                    body,
                    status,
                    stream,
                    &route.deployment,
